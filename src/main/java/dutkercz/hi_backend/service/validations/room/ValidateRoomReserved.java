...
@Component
public class ValidateRoomReserved implements RoomValidations {
    @Override
    public void validateRoom(StayRequestDto requestDto, Room room) {
        if (room.getStatus().equals(RoomStatusEnum.RESERVED)) {
            throw new BusinessException("Room are reserved");
        }
    }
}
